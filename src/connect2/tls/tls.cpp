...
std::optional<Error> tls::connection(const char *connection_host, uint16_t connection_port, const char *destination_host, uint16_t destination_port) {
    ...
    log_debug(connect2, "Starting SSL handshake");
    int status;
    InitContexts ctxs;
    ...
    if (!ctxs.is_valid()) {
        log_error(connect2, "Not enough mem for SSL");
        return Error::Memory;
    }
    ...
    log_debug(connect2, "Loaded certs");
    ...
    if ((status = mbedtls_plain_connect(&net_context, connection_host, connection_port)) != 0) {
        log_info(connect2, "ssl handshake failed with: %d", status);
    ...
    if ((status = mbedtls_ssl_get_verify_result(&ssl_context)) != 0) {
        log_info(connect2, "SSL error");
        return Error::Tls;
    }

    log_debug(connect2, "SSL done");
...
std::variant<size_t, Error> tls::tx(const uint8_t *send_buffer, size_t data_len) {
    ...
    if (status <= 0) {
        log_info(connect2, "ssl write failed with: %d", status);
        if (net_context.timeout_happened) {
            return Error::Timeout;
        } else {
